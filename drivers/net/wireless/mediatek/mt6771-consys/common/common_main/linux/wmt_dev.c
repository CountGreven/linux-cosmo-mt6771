...
#define REFRESH_TIME    300	/* sec */
	...
	static struct __kernel_old_timeval s_query_time;

	INT32 temp_table[HISTORY_NUM];
	INT32 idx_temp_table;
	struct __kernel_old_timeval query_time;

	struct __kernel_old_timeval now_time;
	...
	osal_memcpy(&query_time, &s_query_time, sizeof(struct __kernel_old_timeval));
